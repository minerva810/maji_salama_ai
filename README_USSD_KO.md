...
주민은 전화 앱에서  USSD 단축 코드를 입력해 서비스에
접속합니다. 세션 기반 메뉴에서 등록 우물 ID를 입력하면
...
이 섹션에서는 개발 및 테스트를 위해 프로젝트를 로컬 환경에서 실행하는
방법을 설명합니다.
...
``` bash
conda create -n maji_salama python=3.11
conda activate maji_salama
```
